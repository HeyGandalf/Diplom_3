...
class BasePage:
    ...
    def wait_for_element(self, locator):
        WebDriverWait(self.driver, timeout=10).until(EC.presence_of_element_located(locator))
    ...
    @allure.step('Проверка, что элемент кликабелен через JavaScript')
    def check_element_clickable_js(self, locator, timeout=10):
    ...
    @allure.step('Формирование локатора для поиска заказа')
    def get_order_number_locator(self, order_id):
        return (By.XPATH, f"//p[text()='#0{order_id}']")


    @allure.step('Формирование локатора для заказа в работе')
    ...
    @allure.title('Добавление текста в форму')
    ...
    @allure.step('Получение текста элемента через JS')
    ...
    @allure.step('Ожидание изменения ID')
    ...
    def check_element_not_visible(self, locator, timeout=10):
        try:
            WebDriverWait(self.driver, timeout).until(
